...
public class MainController {
    ...
    public String qrcode_rent(@PathVariable("state")String state, Model model, @AuthenticationPrincipal PrincipalDetails principalDetails) {
        ...
        Holder holderEntity = holderService.빈거치대중제일처음거치대();

        model.addAttribute("booth",booth);
        model.addAttribute("state",state);
        model.addAttribute("holder",holderEntity);
    ...
    @ResponseBody
    public void get_umbrella_info(String umbrella_number, @AuthenticationPrincipal PrincipalDetails principalDetails, Model model){
        boolean check;
        ...
        if(userEntity.isState() == false) {
            Holder holderEntity = holderService.우산아이디로거치대정보찾기(Integer.parseInt(umbrella_number));

            if(voucherEntity.getVoucherStartDate() == null) {
                rentalService.이용권구매후첫대여(voucherEntity.getUser().getId(), holderEntity.getId(), Integer.parseInt(umbrella_number), principalDetails.getBooth().getId());
            }
            else {
                rentalService.대여하기(voucherEntity.getUser().getId(), holderEntity.getId(), Integer.parseInt(umbrella_number), principalDetails.getBooth().getId());
            }
