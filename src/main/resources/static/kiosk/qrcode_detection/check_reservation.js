function check_reservation() {
    //폼에 빈값이 들어오는지 체크필요
    var reservation_code=document.getElementById("reservation_code_input").value;
    if(!reservation_code){
        return;
    }
    ...
    $.ajax({
            type:"post",
            url:"/main/rental/check_qrcode",
            data:{"qrcode_number":reservation_code},
            contentType:"application/x-www-form-urlencoded;charset=utf-8",
            dataType:"json",
            success: function (data){
                console.log("예약번호보내기 성공")
                if(data['check']) {
